...
###
### Anders K. Krabberød 18. june 2019
###########

library(readr)
library(tidyverse)

monopolet <- read_delim("Vinmonopolet_norwegian.csv",
                        ";", escape_double = FALSE, 
...
monopolet_renamed$country <- monopolet_renamed$country %>% 
  str_replace("Amer. Jomfruøy.","U.S. Virgin Islands") %>% 
  ...
  str_replace("Sør-Afrika","South Africa") %>% 
  str_replace("Sør-Korea","South Korea") %>% 
...
monopolet_renamed$type <-monopolet_renamed$type %>% 
  ...
  str_replace("Champagne, rosé","Champagne")  %>% 
  ...
  str_replace("Musserende vin, rosé","Sparkling wine")  %>% 
  str_replace("Perlende vin, hvit","Beading wine")  %>% 
  str_replace("Perlende vin, rosé","Beading wine")  %>% 
  str_replace("Perlende vin, rød ","Beading wine")  %>% 
  str_replace("Rosévin","Rosé wine")  %>% 
  str_replace("Rødvin","Red wine")
#### calculate apk ####
...
selection.list <- paste(c("Champagne", "White wine", 
                          ...
                          "Sparkling wine","Rosé wine"), collapse = '|')
...
monopolet_selection<-monopolet_selection %>% select("product_name","district",
                                                    ...
                                                    "apk","taste_body","taste_freshness","taste_tanins",
                                                    "taste_bitternes","taste_sweetnes","country",
                                                    "alcohol","product_url","fairtrade")
